...
static void settingsIncoming(AsyncWebServerRequest *request, const String &body)
{
    ...
    if (data["haldexGeneration"].is<uint8_t>())
    {
        ...
        if (generation == 1 || generation == 2 || generation == 4 || generation == 50 || generation == 51 || generation == 41)
        {
            ...
            // Do NOT touch lastMode here. haldexGeneration values (1/2/4/41/50/51)
            // are a different namespace from the drive-mode enum (0-5). Writing a
            // generation number into lastMode corrupts the persisted drive mode:
            // on reboot the load switch sees e.g. 41, falls through to default, and
            // the car boots in MODE_FWD instead of the mode the user selected.
        }
    }
